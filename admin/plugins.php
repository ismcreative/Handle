	...
	<div id="page_wrap" style="text-align: center;">
		...
		<?php foreach ($minecraft->get_plugins() as $plugin) {
			...
			if($plugin['enabled'] == "1") {
				...
				if($plugin['name'] == "CraftAPI") {
					$url = "<p>You cannot disable CraftAPI</p>";
				} else {
				$url = "<a href='plugins.php?action=disable&id=".$plugin['name']."'>Disable Plugin</a>";
			}
			} else {
				$img = "<img src='images/icons/cross.png'>";
				$url = "<a href='plugins.php?action=enable&id=".$plugin['name']."'>Enable Plugin</a>";
			}
			echo "<tr>";
